...
public:
   ...
   void keyEvent(int key, bool fDown, bool spec);
   void keyEvent();

   // changing keys
   void changeKey(int toChange, KEYBIND changeTo);
   void triggerChangeKey(int toChange);
   // Current frame rate
   ...
   KEYBIND  getDownKey()  const { return downKey;  };
   KEYBIND  getUpKey()    const { return upKey;    };
   KEYBIND  getLeftKey()  const { return leftKey;  };
   KEYBIND  getRightKey() const { return rightKey; };
   KEYBIND  getSpaceKey() const { return spaceKey; };

   static void *p;                   // for client
   static void (*callBack)(const Interface *, void *);

   static int          changeBind;
...
private:
   ...
   static KEYBIND downKey;
   static KEYBIND upKey;
   static KEYBIND leftKey;
   static KEYBIND rightKey;
   static KEYBIND spaceKey;
};
