...
void keyDownCallback(int key, int x, int y)
{
   ...
   Interface ui;
   KEYBIND keyCode{key, true};
   if (Interface::changeBind >= 0)
   {
     ui.changeKey(Interface::changeBind, keyCode);
     Interface::changeBind = -1;
     ui.keyEvent(13, true, true);
     return;
   }
   ui.keyEvent(key, true /*fDown*/, true);
}
...
void keyUpCallback(int key, int x, int y)
{
   ...
   // so we are actually getting the same version as in the constructor.
   Interface ui;
   ui.keyEvent(key, false /*fDown*/, true);
}
...
void keyboardCallback(unsigned char key, int x, int y)
{
   ...
   ui.keyEvent(key, true /*fDown*/, false);
}

/***************************************************************
 * KEYBOARD UP CALLBACK
 * Generic callback to a regular ascii keyboard up event, such
 * as the space bar or the letter 'q'
 ***************************************************************/
void keyboardUpCallback(unsigned char key, int x, int y)
{
  ...
  // so we are actually getting the same version as in the constructor.
  Interface ui;
  ui.keyEvent(key, false /*fDown*/, false);
}
 ...
 *           spec    special key
 ****************************************************************/
void Interface::keyEvent(int key, bool fDown, bool spec)
{
   if (key == downKey.key && spec == downKey.spec)
   {
     isDownPress = fDown;
   }
   else if (key == upKey.key && spec == upKey.spec)
   {
     isUpPress = fDown;
   }
   else if (key == rightKey.key && spec == rightKey.spec)
   {
     isRightPress = fDown;
   }
   else if (key == leftKey.key && spec == leftKey.spec)
   {
     isLeftPress = fDown;
   }
   else if (key == spaceKey.key && spec == spaceKey.spec)
   {
     isSpacePress = fDown;
   }
   else
...
void Interface::changeKey(int toChange, KEYBIND changeTo)
{
  switch(toChange)
  {
  case 0:
...
void Interface::triggerChangeKey(int toChange)
{
  changeBind = toChange;
}
...
int          Interface::changeBind   = -1;
KEYBIND      Interface::downKey      = {GLUT_KEY_DOWN, true};
KEYBIND      Interface::upKey        = {GLUT_KEY_UP, true};
KEYBIND      Interface::leftKey      = {GLUT_KEY_LEFT, true};
KEYBIND      Interface::rightKey     = {GLUT_KEY_RIGHT, true};
KEYBIND      Interface::spaceKey     = {' ', false};
